...
istar.setupMetamodel = function (metamodel) {
    ...
    _.each(metamodel.nodes, function(node) {istar._setupBasicType(node, metamodel);});
    _.each(metamodel.containers, function(node) {istar._setupBasicType(node, metamodel);});
    ...
    _.each(metamodel.containerLinks, function(node) {istar._setupBasicLink(node, metamodel);});
    _.each(metamodel.containerLinks, function(node) {istar._setupLinkBetweenActors(node, metamodel);});
    _.each(metamodel.nodeLinks, function(node) {istar._setupBasicLink(node, metamodel);});
    _.each(metamodel.nodeLinks, function(node) {istar._setupLinkBetweenNodes(node, metamodel);});

    istar._createIsActorLinkFunction(metamodel);

    joint.dia.Cell.prototype.isKindOfInnerElement = function () {return this.isGoal() || this.isQuality() || this.isTask() || this.isResource();};
    joint.dia.Cell.prototype.isKindOfActor = function () {
        return this.isActor() || this.isRole() || this.isAgent();
    };


    joint.dia.Cell.prototype.isDependencyLink = function () {return this.attributes.type === istar.linkTypes.dependency.name;};
...
istar._createIsActorLinkFunction = function (metamodel) {
    ...
    _.each(metamodel.containerLinks, function(link) {namesOfActorLinkTypes.push('is' + link.name);});
